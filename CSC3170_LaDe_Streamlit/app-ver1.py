...
def search_filters(conn: sqlite3.Connection) -> dict[str, Any]:
    st.subheader("Filters")

    scenario = st.multiselect("Scenario", options=["pickup", "delivery"], default=["pickup", "delivery"])
    ...
    selected_regions = st.multiselect("region_id", options=region_options, default=region_options)
    ...
    date_range = st.date_input("Accept date range", value=(min_dt, max_dt), min_value=min_dt, max_value=max_dt)
...
def page_search(conn: sqlite3.Connection) -> None:
    st.subheader("Search")
    tab1, tab2, tab3 = st.tabs(["Orders", "Couriers", "GPS timeline"])

    with tab1:
        flt = search_filters(conn)
        st.divider()
        left, right = st.columns([2, 1])
        with left:
            keyword = st.text_input("Keyword (order_id / courier_id)", value="")
        with right:
            aoi_types = df_query(conn, "SELECT DISTINCT aoi_type FROM aoi WHERE aoi_type IS NOT NULL ORDER BY aoi_type")
            aoi_options = ["(all)"] + [str(int(x)) for x in aoi_types["aoi_type"].tolist()] if not aoi_types.empty else ["(all)"]
            aoi_type = st.selectbox("AOI type", options=aoi_options)
    ...
    with tab1:
        with st.form("add_courier", clear_on_submit=True):
            ...
            if submitted:
                ...
                elif courier_phone.strip() and not courier_phone.strip().isdigit():
                    st.error("courier_phone must be digits only.")
                else:
                    try:
                        existing = conn.execute(
                            "SELECT courier_id FROM courier WHERE courier_id = ?",
                            (int(courier_id.strip()),),
                        ).fetchone()
                        if existing:
                            st.error(f"courier_id {courier_id.strip()} already exists in the database.")
                        else:
                            city_id = ensure_city(conn)
                            exec_write(
                                conn,
                                "INSERT INTO courier(courier_id, city_id) VALUES (?, ?)",
        ...
        if df.empty:
            ...
            with col1:
                ...
                if st.button("Update", type="primary"):
                    if new_courier.strip() and not new_courier.strip().isdigit():
                        st.error("courier_id must be a number.")
                    elif new_aoi.strip() and not new_aoi.strip().isdigit():
                        st.error("aoi_id must be a number.")
                    else:
                        try:
                            if new_courier.strip().isdigit():
                                courier_exists = conn.execute(
                                    "SELECT courier_id FROM courier WHERE courier_id = ?",
                                    (int(new_courier.strip()),),
                                ).fetchone()
                                if not courier_exists:
                                    st.error(f"courier_id {new_courier.strip()} does not exist in the database.")
                                else:
                                    city_id = ensure_city(conn)
                                    exec_write(
                                        conn,
...
def main() -> None:
    ...
    page = st.sidebar.radio("", options=["Overview", "Search", "Admin", "SQL Console"], index=0)
